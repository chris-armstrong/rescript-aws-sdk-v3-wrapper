...
type ulimit = {
  ...
  @ocaml.doc("<p>The <code>type</code> of the <code>ulimit</code>.</p>") name: string_,
  @ocaml.doc("<p>The hard limit for the <code>ulimit</code> type.</p>") hardLimit: integer_,
}
...
type resourceRequirement = {
  @ocaml.doc("<p>The type of resource to assign to a container. The supported resources include <code>GPU</code>,
    <code>MEMORY</code>, and <code>VCPU</code>.</p>")
  @as("type")
  type_: resourceType,
  @ocaml.doc("<p>The quantity of the specified resource to reserve for the container. The values vary based on the
...
type nodePropertiesSummary = {
  @ocaml.doc("<p>The node index for the node. Node index numbering begins at zero. This index is also available on the node with
...
@ocaml.doc("<p>An object representing the details of a multi-node parallel job node.</p>")
type nodeDetails = {
  @ocaml.doc(
    "<p>Specifies whether the current node is the main node for a multi-node parallel job.</p>"
  )
...
@ocaml.doc("<p>Details on a Docker volume mount point that's used in a job's container properties. This parameter maps to
...
type mountPoint = {
  @ocaml.doc("<p>The name of the volume to mount.</p>") sourceVolume: option<string_>,
  @ocaml.doc("<p>If this value is <code>true</code>, the container has read-only access to the volume. Otherwise, the container
   can write to the volume. The default value is <code>false</code>.</p>")
  readOnly: option<boolean_>,
  @ocaml.doc("<p>The path on the container where the host volume is mounted.</p>")
  containerPath: option<string_>,
}
type logConfigurationOptionsMap = Js.Dict.t<string_>
@ocaml.doc("<p>An object representing a launch template associated with a compute resource. You must specify either the launch
...
type launchTemplateSpecification = {
  ...
  @ocaml.doc("<p>The name of the launch template.</p>") launchTemplateName: option<string_>,
  @ocaml.doc("<p>The ID of the launch template.</p>") launchTemplateId: option<string_>,
}
@ocaml.doc("<p>A key-value pair object.</p>")
type keyValuePair = {
  @ocaml.doc("<p>The value of the key-value pair. For environment variables, this is the value of the environment
   variable.</p>")
...
type host = {
  @ocaml.doc("<p>The path on the host container instance that's presented to the container. If this parameter is empty, then the
...
type evaluateOnExit = {
  @ocaml.doc("<p>Specifies the action to take if all of the specified conditions (<code>onStatusReason</code>,
...
@ocaml.doc("<p>An object representing summary details of a container within a job.</p>")
type containerSummary = {
  @ocaml.doc("<p>A short (255 max characters) human-readable string to provide additional details about a running or stopped
   container.</p>")
  reason: option<string_>,
  @ocaml.doc("<p>The exit code to return upon completion.</p>") exitCode: option<integer_>,
}
@ocaml.doc("<p>The order in which compute environments are tried for job placement within a queue. Compute environments are
...
type computeEnvironmentOrder = {
  ...
  @ocaml.doc("<p>The order of the compute environment. Compute environments are tried in ascending order. For example, if two
...
@ocaml.doc("<p>An object representing the array properties of a job.</p>")
type arrayPropertiesSummary = {
  @ocaml.doc("<p>The job index within the array that's associated with this job. This parameter is returned for children of array
   jobs.</p>")
...
type tmpfs = {
  ...
  @ocaml.doc("<p>The size (in MiB) of the tmpfs volume.</p>") size: integer_,
...
type jobSummary = {
  ...
  @ocaml.doc("<p>The Unix timestamp for when the job was stopped (when the job transitioned from the <code>RUNNING</code> state
   to a terminal state, such as <code>SUCCEEDED</code> or <code>FAILED</code>).</p>")
  stoppedAt: option<long>,
  @ocaml.doc("<p>The Unix timestamp for when the job was started (when the job transitioned from the <code>STARTING</code> state
...
type arrayPropertiesDetail = {
  @ocaml.doc("<p>The job index within the array that's associated with this job. This parameter is returned for array job
   children.</p>")
  index: option<integer_>,
  @ocaml.doc("<p>The size of the array job. This parameter is returned for parent array jobs.</p>")
  size: option<integer_>,
  @ocaml.doc("<p>A summary of the number of array job children in each available job status. This parameter is returned for
   parent array jobs.</p>")
...
type retryStrategy = {
  ...
  @ocaml.doc("<p>The number of times to move a job to the <code>RUNNABLE</code> status. You can specify between 1 and 10
...
type logConfiguration = {
 ...
 container instance and run the following command: <code>sudo docker version | grep \"Server API version\"</code>
  ...
  @ocaml.doc("<p>The log driver to use for the container. The valid values listed for this parameter are log drivers that the
...
type jobQueueDetail = {
  ...
  @ocaml.doc("<p>The priority of the job queue. Job queues with a higher priority (or a higher integer value for the
...
type containerOverrides = {
  @ocaml.doc("<p>The type and amount of resources to assign to a container. This overrides the settings in the job definition.
  ...
  @ocaml.doc("<p>The environment variables to send to the container. You can add new environment variables, which are added to
...
type computeResource = {
  ...
  @ocaml.doc("<p>The instances types that can be launched. You can specify instance families to launch any instance type within
...
@ocaml.doc("<p>An object representing the details of a container that's part of a job attempt.</p>")
type attemptContainerDetail = {
  ...
  @ocaml.doc("<p>The Amazon Resource Name (ARN) of the Amazon ECS task that's associated with the job attempt. Each container attempt receives a task
...
type nodePropertyOverride = {
  @ocaml.doc("<p>The overrides that should be sent to a node range.</p>")
  containerOverrides: option<containerOverrides>,
  @ocaml.doc("<p>The range of nodes, using node index values, that's used to override. A range of <code>0:3</code> indicates
...
type linuxParameters = {
  @ocaml.doc("<p>This allows you to tune a container's memory swappiness behavior. A <code>swappiness</code> value of
...
type attemptDetail = {
  ...
  statusReason: option<string_>,
  @ocaml.doc("<p>The Unix timestamp (in milliseconds) for when the attempt was stopped (when the attempt transitioned from the
    <code>RUNNING</code> state to a terminal state, such as <code>SUCCEEDED</code> or <code>FAILED</code>).</p>")
  stoppedAt: option<long>,
  @ocaml.doc("<p>The Unix timestamp (in milliseconds) for when the attempt was started (when the attempt transitioned from the
...
type containerProperties = {
 ...
 container instance and run the following command: <code>sudo docker version | grep \"Server API version\"</code>
  ...
  @ocaml.doc("<p>The type and amount of resources to assign to a container. The supported resources include <code>GPU</code>,
    <code>MEMORY</code>, and <code>VCPU</code>.</p>")
  resourceRequirements: option<resourceRequirements>,
  ...
  instanceType: option<string_>,
  @ocaml.doc("<p>The user name to use inside the container. This parameter maps to <code>User</code> in the
  ...
  @ocaml.doc("<p>The mount points for data volumes in your container. This parameter maps to <code>Volumes</code> in the
  ...
  @ocaml.doc("<p>The environment variables to pass to a container. This parameter maps to <code>Env</code> in the
...
type containerDetail = {
 ...
 container instance and run the following command: <code>sudo docker version | grep \"Server API version\"</code>
  ...
  @ocaml.doc("<p>The type and amount of resources to assign to a container. The supported resources include <code>GPU</code>,
    <code>MEMORY</code>, and <code>VCPU</code>.</p>")
  resourceRequirements: option<resourceRequirements>,
  @ocaml.doc("<p>The network interfaces associated with the job.</p>")
  networkInterfaces: option<networkInterfaceList>,
  ...
  @ocaml.doc("<p>The name of the CloudWatch Logs log stream associated with the container. The log group for Batch jobs is
    ...
    <code>RUNNING</code> status.</p>")
  logStreamName: option<string_>,
  @ocaml.doc("<p>The Amazon Resource Name (ARN) of the Amazon ECS task that's associated with the container job. Each container attempt receives a task
   ARN when they reach the <code>STARTING</code> status.</p>")
  taskArn: option<string_>,
  @ocaml.doc(
 ...
 convention is reserved for variables that are set by the Batch service.</p>
  ...
  vcpus: option<integer_>,
  @ocaml.doc("<p>The image used to start the container.</p>") image: option<string_>,
}
...
type nodeRangeProperty = {
  @ocaml.doc("<p>The container details for the node range.</p>")
  container: option<containerProperties>,
  @ocaml.doc("<p>The range of nodes, using node index values. A range of <code>0:3</code> indicates nodes with index values of
...
type nodeOverrides = {
  @ocaml.doc("<p>The node property overrides for the job.</p>")
  nodePropertyOverrides: option<nodePropertyOverrides>,
  @ocaml.doc("<p>The number of nodes to use with a multi-node parallel job. This value overrides the number of nodes that are
...
type nodeProperties = {
  ...
  @ocaml.doc("<p>Specifies the node index for the main node of a multi-node parallel job. This node index value must be fewer
...
type jobDetail = {
  ...
  @ocaml.doc("<p>The tags applied to the job.</p>") tags: option<tagrisTagsMap>,
  @ocaml.doc("<p>The timeout configuration for the job.</p>") timeout: option<jobTimeout>,
  @ocaml.doc("<p>The array properties of the job, if it is an array job.</p>")
  ...
  @ocaml.doc("<p>Additional parameters passed to the job that replace parameter substitution placeholders or override any
   corresponding parameter defaults from the job definition.</p>")
  parameters: option<parametersMap>,
  @ocaml.doc("<p>The job definition that's used by this job.</p>") jobDefinition: string_,
  @ocaml.doc("<p>A list of job IDs that this job depends on.</p>")
  dependsOn: option<jobDependencyList>,
  @ocaml.doc("<p>The Unix timestamp (in milliseconds) for when the job was stopped (when the job transitioned from the
    <code>RUNNING</code> state to a terminal state, such as <code>SUCCEEDED</code> or <code>FAILED</code>).</p>")
  stoppedAt: option<long>,
  @ocaml.doc("<p>The Unix timestamp (in milliseconds) for when the job was started (when the job transitioned from the
  ...
  startedAt: long,
  @ocaml.doc("<p>The retry strategy to use for this job if an attempt fails.</p>")
  ...
  jobQueue: string_,
  @ocaml.doc("<p>The ID for the job.</p>") jobId: string_,
  @ocaml.doc("<p>The name of the job.</p>") jobName: string_,
  @ocaml.doc("<p>The Amazon Resource Name (ARN) of the job.</p>") jobArn: option<string_>,
}
...
type jobDefinition = {
  ...
  @ocaml.doc("<p>Specifies whether to propagate the tags from the job or job definition to the corresponding Amazon ECS task. If no
   ...
   from the job and job definition is over 50, the job is moved to the <code>FAILED</code> state.</p>")
  propagateTags: option<boolean_>,
  @ocaml.doc("<p>The tags applied to the job definition.</p>") tags: option<tagrisTagsMap>,
  @ocaml.doc("<p>An object with various properties specific to multi-node parallel jobs.</p>
         <note>
  ...
  @ocaml.doc("<p>The revision of the job definition.</p>") revision: integer_,
  @ocaml.doc("<p>The Amazon Resource Name (ARN) for the job definition.</p>")
  jobDefinitionArn: string_,
  @ocaml.doc("<p>The name of the job definition.</p>") jobDefinitionName: string_,
}
...
module UpdateJobQueue = {
  ...
  type request = {
    @ocaml.doc("<p>Details the set of compute environments mapped to a job queue and their order relative to each other. This is
   ...
   queue can finish.</p>")
    ...
    @ocaml.doc("<p>The name or the Amazon Resource Name (ARN) of the job queue.</p>")
    jobQueue: string_,
  }
  type response = {
    @ocaml.doc("<p>The Amazon Resource Name (ARN) of the job queue.</p>")
...
module ListJobs = {
 ...
 retrieve the next items in a list and not for other programmatic purposes.</p>
    ...
    nextToken: option<string_>,
    @ocaml.doc("<p>The maximum number of results returned by <code>ListJobs</code> in paginated output. When this parameter is
   ...
   nodes that are associated with the specified job.</p>")
    ...
    @ocaml.doc("<p>The job ID for an array job. Specifying an array job ID with this parameter lists all child jobs from within the
   specified array.</p>")
  ...
  type response = {
    @ocaml.doc("<p>The <code>nextToken</code> value to include in a future <code>ListJobs</code> request. When the results of a
   ...
   results. This value is <code>null</code> when there are no more results to return.</p>")
    nextToken: option<string_>,
    @ocaml.doc("<p>A list of job summaries that match the request.</p>")
...
module DescribeJobQueues = {
  ...
  type request = {
    @ocaml.doc("<p>The <code>nextToken</code> value returned from a previous paginated <code>DescribeJobQueues</code> request where
    <code>maxResults</code> was used and the results exceeded the value of that parameter. Pagination continues from the
   end of the previous results that returned the <code>nextToken</code> value. This value is <code>null</code> when
   there are no more results to return.</p>
         <note>
 ...
 retrieve the next items in a list and not for other programmatic purposes.</p>
    ...
    nextToken: option<string_>,
    @ocaml.doc("<p>The maximum number of results returned by <code>DescribeJobQueues</code> in paginated output. When this
  ...
  type response = {
    @ocaml.doc("<p>The <code>nextToken</code> value to include in a future <code>DescribeJobQueues</code> request. When the results
   ...
   next page of results. This value is <code>null</code> when there are no more results to return.</p>")
    nextToken: option<string_>,
    @ocaml.doc("<p>The list of job queues.</p>") jobQueues: option<jobQueueDetailList>,
  }
...
module DescribeComputeEnvironments = {
  ...
  type request = {
    @ocaml.doc("<p>The <code>nextToken</code> value returned from a previous paginated <code>DescribeComputeEnvironments</code>
   request where <code>maxResults</code> was used and the results exceeded the value of that parameter. Pagination
   continues from the end of the previous results that returned the <code>nextToken</code> value. This value is
 ...
 retrieve the next items in a list and not for other programmatic purposes.</p>
    ...
    nextToken: option<string_>,
    @ocaml.doc("<p>The maximum number of cluster results returned by <code>DescribeComputeEnvironments</code> in paginated output.
...
module SubmitJob = {
  ...
  type request = {
   ...
   overrides the tag propagation setting in the job definition.</p>")
    propagateTags: option<boolean_>,
    @ocaml.doc("<p>The retry strategy to use for failed jobs from this <a>SubmitJob</a> operation. When a retry strategy
   ...
   override.</p>")
    containerOverrides: option<containerOverrides>,
    @ocaml.doc("<p>Additional parameters passed to the job that replace parameter substitution placeholders that are set in the job
   ...
   override any corresponding parameter defaults from the job definition.</p>")
    parameters: option<parametersMap>,
    @ocaml.doc("<p>The job definition used by this job. This value can be one of <code>name</code>, <code>name:revision</code>, or
   ...
   revision is used.</p>")
    jobDefinition: string_,
    @ocaml.doc("<p>A list of dependencies for the job. A job can depend upon a maximum of 20 jobs. You can specify a
  ...
  type response = {
    @ocaml.doc("<p>The unique identifier for the job.</p>") jobId: string_,
    @ocaml.doc("<p>The name of the job.</p>") jobName: string_,
    @ocaml.doc("<p>The Amazon Resource Name (ARN) for the job.</p>") jobArn: option<string_>,
  }
...
module RegisterJobDefinition = {
  ...
  type request = {
   ...
   from the job and job definition is over 50, the job is moved to the <code>FAILED</code> state.</p>")
    propagateTags: option<boolean_>,
    @ocaml.doc("<p>The retry strategy to use for failed jobs that are submitted with this job definition. Any retry strategy that's
   ...
   <code>nodeProperties</code>.</p>
    ...
    nodeProperties: option<nodeProperties>,
    @ocaml.doc("<p>An object with various properties specific to single-node container-based jobs. If the job definition's
...
module DescribeJobs = {
  ...
  type request = {@ocaml.doc("<p>A list of up to 100 job IDs.</p>") jobs: stringList}
  type response = {@ocaml.doc("<p>The list of jobs.</p>") jobs: option<jobDetailList>}
...
module DescribeJobDefinitions = {
  ...
  type request = {
    @ocaml.doc("<p>The <code>nextToken</code> value returned from a previous paginated <code>DescribeJobDefinitions</code> request
 ...
 retrieve the next items in a list and not for other programmatic purposes.</p>
    ...
    nextToken: option<string_>,
    @ocaml.doc("<p>The status used to filter job definitions.</p>") status: option<string_>,
    @ocaml.doc("<p>The name of the job definition to describe.</p>")
    jobDefinitionName: option<string_>,
    @ocaml.doc("<p>The maximum number of results returned by <code>DescribeJobDefinitions</code> in paginated output. When this
  ...
  type response = {
    @ocaml.doc("<p>The <code>nextToken</code> value to include in a future <code>DescribeJobDefinitions</code> request. When the
   results of a <code>DescribeJobDefinitions</code> request exceed <code>maxResults</code>, this value can be used to
   retrieve the next page of results. This value is <code>null</code> when there are no more results to return.</p>")
    nextToken: option<string_>,
    @ocaml.doc("<p>The list of job definitions.</p>") jobDefinitions: option<jobDefinitionList>,
  }
