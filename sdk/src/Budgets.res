...
@ocaml.doc("<p> The comparison operator of a notification. Currently the service supports the following operators:</p>
		       <p>
...
@ocaml.doc("<p>The account ID of the user. It should be a 12-digit number.</p>")
...
type timePeriod = {
  @ocaml.doc("<p>The end date for a budget. If you didn't specify an end date, AWS set your end date to <code>06/15/87 00:00 UTC</code>. The defaults are the same for the AWS Billing and Cost Management console and the API.</p>
...
type subscriber = {
  @ocaml.doc("<p>The address that AWS sends budget notifications to, either an SNS topic or an email.</p>
		       <p>When you create a subscriber, the value of <code>Address</code> can't contain line breaks.</p>")
  @as("Address")
  address: subscriberAddress,
  @ocaml.doc("<p>The type of notification that AWS sends to a subscriber.</p>")
  @as("SubscriptionType")
  subscriptionType: subscriptionType,
}
@ocaml.doc("<p>The amount of cost or usage that is measured for a budget.</p>
...
@ocaml.doc("<p>A notification that is associated with a budget. A budget can have up to ten notifications. </p>
...
type notification = {
  ...
  @ocaml.doc("<p>The comparison that is used for this notification.</p>") @as("ComparisonOperator")
...
@ocaml.doc("<p>The spend objects that are associated with this budget. The <code>actualSpend</code> tracks how much you've used, cost, usage, RI units, or Savings Plans units and the <code>forecastedSpend</code> tracks how much you are predicted to spend based on your historical usage profile.</p>
...
type budgetedAndActualAmounts = {
  @ocaml.doc("<p>The time period covered by this budget comparison.</p>") @as("TimePeriod")
...
type notificationWithSubscribers = {
  ...
  @ocaml.doc("<p>The notification that is associated with a budget.</p>") @as("Notification")
...
type budget = {
  ...
  @ocaml.doc("<p>The period of time that is covered by a budget. The period has a start date and an end date. The start date must come before the end date. The end date must come before <code>06/15/87 00:00 UTC</code>. </p>
  ...
  @ocaml.doc("<p>The cost filters, such as service or tag, that are applied to a budget.</p>
  ...
  @ocaml.doc("<p>The total amount of cost, usage, RI utilization, RI coverage, Savings Plans utilization, or Savings Plans coverage that you want to track with your budget.</p>
		       <p>
...
@ocaml.doc("<p>The AWS Budgets API enables you to use AWS Budgets to plan your service usage, service costs, and instance reservations. The API reference provides descriptions, syntax, and usage examples for each of the actions and data types for AWS Budgets. </p>
