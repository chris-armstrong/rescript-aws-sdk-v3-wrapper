...
@ocaml.doc("<p>Determines the level of detail about provisioned throughput consumption that is returned in the response:</p>
         <ul>
            <li>
               <p>
...
@ocaml.doc("<p>Describes a tag. A tag is a key-value pair. You can add up to 50 tags to a single DynamoDB table.
      </p>
...
type tag = {
  ...
  @ocaml.doc("<p>The key of the tag. Tag keys are case sensitive. Each DynamoDB table can only have up to
...
type ssespecification = {
  @ocaml.doc("<p>The AWS KMS customer master key (CMK) that should be used for the AWS KMS encryption. To
  ...
  @ocaml.doc("<p>Indicates whether server-side encryption is done using an AWS managed CMK or an AWS owned CMK. If enabled (true),
...
type ssedescription = {
  @ocaml.doc("<p>Indicates the time, in UNIX epoch date format, when DynamoDB detected that the table's
  ...
  @ocaml.doc("<p>The AWS KMS customer master key (CMK) ARN used for the AWS KMS encryption.</p>")
  @as("KMSMasterKeyArn")
  ...
  @as("SSEType")
  ssetype: option<ssetype>,
  @ocaml.doc("<p>Represents the current state of server-side encryption. The only supported values are:</p>
         <ul>
            <li>
               <p>
...
@ocaml.doc("<p>Replica-specific provisioned throughput settings. If not specified, uses the
         source table's provisioned throughput settings.</p>")
type provisionedThroughputOverride = {
  @ocaml.doc("<p>Replica-specific read capacity units. If not specified, uses the source table's
...
type provisionedThroughputDescription = {
  ...
  @ocaml.doc("<p>The maximum number of strongly consistent reads consumed per second before DynamoDB returns a
  ...
  @ocaml.doc("<p>The number of provisioned throughput decreases for this table during this UTC calendar day.
...
@ocaml.doc("<p>Represents the provisioned throughput settings for a specified table or index. The settings
...
type pointInTimeRecoveryDescription = {
  ...
  @ocaml.doc("<p>Specifies the earliest point in time you can restore your table to. You can restore your
...
@ocaml.doc("<p>Represents <i>a single element</i> of a key schema. A key schema specifies the attributes
...
type exportDescription = {
  ...
  @ocaml.doc("<p>The ID of the AWS KMS managed key used to encrypt the S3 bucket where export data is
            stored (if applicable).</p>")
  @as("S3SseKmsKeyId")
  s3SseKmsKeyId: option<s3SseKmsKeyId>,
  @ocaml.doc("<p>Type of encryption used on the bucket where export data is stored. Valid values
  ...
  @ocaml.doc("<p>The ID of the AWS account that owns the bucket containing the export.</p>")
  @as("S3BucketOwner")
...
type billingModeSummary = {
  ...
  @ocaml.doc("<p>Controls how you are charged for read and write throughput and how you manage capacity. This setting can be changed later.</p>
         <ul>
            <li>
               <p>
...
type backupSummary = {
  ...
  @ocaml.doc("<p>Time at which the automatic on-demand backup created by DynamoDB will expire. This
...
type backupDetails = {
  @ocaml.doc("<p>Time at which the automatic on-demand backup created by DynamoDB will expire. This <code>SYSTEM</code>
...
type autoScalingTargetTrackingScalingPolicyConfigurationUpdate = {
  ...
  @as("TargetValue")
  targetValue: double,
  @ocaml.doc("<p>The amount of time, in seconds, after a scale out activity completes before another scale out
  ...
  @ocaml.doc("<p>The amount of time, in seconds, after a scale in activity completes before another scale
  ...
  @ocaml.doc("<p>Indicates whether scale in by the target tracking policy is disabled. If the value is true,
...
type archivalSummary = {
  @ocaml.doc("<p>The Amazon Resource Name (ARN) of the backup the table was archived
  ...
  @ocaml.doc("<p>The reason DynamoDB archived the table. Currently, the only
         possible value is:</p>
  ...
  @ocaml.doc("<p>The date and time when table archival was initiated by DynamoDB,
...
type updateGlobalSecondaryIndexAction = {
  @ocaml.doc("<p>Represents the provisioned throughput settings for the specified global secondary index.</p>
         <p>For current minimum and maximum provisioned throughput values, see <a href=\"https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Limits.html\">Service, Account, and Table Quotas</a> in the <i>Amazon DynamoDB Developer Guide</i>.</p>")
...
type replicaGlobalSecondaryIndex = {
  @ocaml.doc("<p>Replica table GSI-specific provisioned throughput. If not specified, uses the
...
@ocaml.doc("<p>Represents the properties of a local secondary index for the table
...
type localSecondaryIndexInfo = {
  ...
  @ocaml.doc("<p>The complete key schema for a local secondary index, which consists of one or more pairs of attribute names and key types:</p>
         <ul>
            <li>
               <p>
                  <code>HASH</code> - partition key</p>
...
type localSecondaryIndexDescription = {
  ...
  @as("Projection")
  projection: option<projection>,
  @ocaml.doc("<p>The complete key schema for the local secondary index, consisting of one or more pairs of attribute names and key types:</p>
         <ul>
            <li>
               <p>
                  <code>HASH</code> - partition key</p>
...
@ocaml.doc("<p>Represents the properties of a global secondary index for the table
     when the backup was created.</p>")
type globalSecondaryIndexInfo = {
  ...
  @ocaml.doc("<p>Represents attributes that are copied (projected) from the table into
  ...
  @as("Projection")
  projection: option<projection>,
  @ocaml.doc("<p>The complete key schema for a global secondary index, which consists of one or more pairs of attribute names and key types:</p>
         <ul>
            <li>
               <p>
                  <code>HASH</code> - partition key</p>
...
type globalSecondaryIndexDescription = {
  ...
  @ocaml.doc("<p>Indicates whether the index is currently backfilling. <i>Backfilling</i> is the process of reading items from 
...
type updateReplicationGroupMemberAction = {
  ...
  @as("ProvisionedThroughputOverride")
  provisionedThroughputOverride: option<provisionedThroughputOverride>,
  @ocaml.doc("<p>The AWS KMS customer master key (CMK) of the replica that should be used for AWS KMS encryption.
...
@ocaml.doc("<p>Represents the auto scaling settings of a global secondary index for a replica
...
type replicaDescription = {
  ...
  @as("ProvisionedThroughputOverride")
  provisionedThroughputOverride: option<provisionedThroughputOverride>,
  @ocaml.doc("<p>The AWS KMS customer master key (CMK) of the replica that will be used for AWS KMS
         encryption.</p>")
  @as("KMSMasterKeyId")
  kmsmasterKeyId: option<kmsmasterKeyId>,
  @ocaml.doc("<p>Specifies the progress of a Create, Update, or Delete action on the replica
         as a percentage.</p>")
...
type globalTableGlobalSecondaryIndexSettingsUpdate = {
  ...
  @ocaml.doc("<p>The maximum number of writes consumed per second before DynamoDB returns a <code>ThrottlingException.</code>
         </p>")
...
type globalSecondaryIndexUpdate = {
  ...
  @ocaml.doc("<p>The parameters required for creating a global secondary index on an existing table:</p>
         <ul>
            <li>
               <p>
                  <code>IndexName </code>
...
type createReplicationGroupMemberAction = {
  ...
  @as("ProvisionedThroughputOverride")
  provisionedThroughputOverride: option<provisionedThroughputOverride>,
  @ocaml.doc("<p>The AWS KMS customer master key (CMK) that should be used for AWS KMS encryption
...
type sourceTableFeatureDetails = {
  ...
  @ocaml.doc("<p>Represents the GSI properties for the table when the backup was created. It includes the
...
type tableDescription = {
  ...
  @ocaml.doc("<p>The global secondary indexes, if any, on the table. Each index is scoped to a given partition key value. Each element is composed of:</p>
         <ul>
            <li>
               <p>
  ...
  @ocaml.doc("<p>The primary key structure for the table. Each <code>KeySchemaElement</code> consists of:</p>
         <ul>
            <li>
               <p>
  ...
  @ocaml.doc("<p>An array of <code>AttributeDefinition</code> objects. Each of these objects describes one attribute
...
type replicaSettingsUpdate = {
  ...
  @ocaml.doc("<p>The maximum number of strongly consistent reads consumed per second before DynamoDB returns a <code>ThrottlingException</code>.
...
type replicaAutoScalingUpdate = {
  ...
  @ocaml.doc("<p>Represents the auto scaling settings of global secondary indexes that will
         be modified.</p>")
...
@ocaml.doc("<p>For the <code>UpdateItem</code> operation, represents the attributes to be modified, the action to
...
type attributeValueUpdate = {
  @ocaml.doc("<p>Specifies how to perform the update. Valid values are <code>PUT</code> (default), <code>DELETE</code>,
...
type update = {
  @ocaml.doc("<p>Use <code>ReturnValuesOnConditionCheckFailure</code> to
  ...
  tableName: tableName,
  @ocaml.doc("<p>An expression that defines one or more attributes to be updated,
      the action to be performed on them, and new value(s) for them.</p>")
  @as("UpdateExpression")
  updateExpression: updateExpression,
  @ocaml.doc("<p>The primary key of the item to be updated. Each element consists of
...
type putRequest = {
  @ocaml.doc("<p>A map of attribute name to attribute values, representing the primary key of an item to
...
type put = {
  ...
  tableName: tableName,
  @ocaml.doc("<p>A map of attribute name to attribute values, representing the primary key
...
type itemCollectionMetrics = {
  @ocaml.doc("<p>An estimate of item collection size, in gigabytes. This value is a two-element array containing a lower bound and an upper bound for the estimate. The estimate includes the size of all the items in the table, plus the size of all attributes projected into all of the local secondary indexes on that table. Use this estimate to measure whether a local secondary index is approaching its size limit.</p>
...
type get = {
  @ocaml.doc("<p>One or more substitution tokens for attribute names in the
        ProjectionExpression parameter.</p>")
  @as("ExpressionAttributeNames")
  expressionAttributeNames: option<expressionAttributeNameMap>,
  @ocaml.doc("<p>A string that identifies one or more attributes of the specified item
  ...
  @as("TableName")
  tableName: tableName,
  @ocaml.doc("<p>A map of attribute names to <code>AttributeValue</code> objects that
...
type expectedAttributeValue = {
  @ocaml.doc("<p>One or more values to evaluate against the supplied attribute. The number of values in the
  ...
  @ocaml.doc("<p>A comparator for evaluating attributes in the <code>AttributeValueList</code>. For example, equals,
  ...
  @ocaml.doc("<p>Causes DynamoDB to evaluate the value before attempting a conditional operation:</p>
         <ul>
            <li>
...
type delete = {
  ...
  tableName: tableName,
  @ocaml.doc("<p>The primary key of the item to be deleted. Each element consists of an
      attribute name and a value for that attribute.</p>")
  @as("Key")
  key: key,
}
@ocaml.doc("<p>Represents a request to perform a check that an item exists or to check the condition of
...
type conditionCheck = {
  ...
  tableName: tableName,
  @ocaml.doc("<p>The primary key of the item to be checked. Each element consists of an
      attribute name and a value for that attribute.</p>")
  @as("Key")
  key: key,
}
@ocaml.doc("<p>Represents the selection criteria for a <code>Query</code> or <code>Scan</code> operation:</p>
         <ul>
            <li>
...
type condition = {
  @ocaml.doc("<p>A comparator for evaluating attributes. For example, equals, greater than, less than, etc.</p>
         <p>The following comparison operators are available:</p>
         <p>
            <code>EQ | NE | LE | LT | GE | GT | NOT_NULL | NULL | CONTAINS | NOT_CONTAINS | BEGINS_WITH | IN | BETWEEN</code>
         </p>
...
@ocaml.doc("<p>An ordered list of errors for each item in the request which caused the transaction
...
type transactGetItem = {
  @ocaml.doc("<p>Contains the primary key that identifies the item to get, together
...
@ocaml.doc("<p>Represents a set of primary keys and, for each key, the attributes to retrieve from the table.</p>
...
type keysAndAttributes = {
  @ocaml.doc("<p>One or more substitution tokens for attribute names in an expression. The following are some use cases for using <code>ExpressionAttributeNames</code>:</p>
         <ul>
            <li>
  ...
  @as("ExpressionAttributeNames")
  expressionAttributeNames: option<expressionAttributeNameMap>,
  @ocaml.doc("<p>A string that identifies one or more attributes to retrieve from the table. These attributes can include scalars,
  ...
  @as("ProjectionExpression")
  projectionExpression: option<projectionExpression>,
  @ocaml.doc("<p>The consistency of a read operation. If set to <code>true</code>, then a strongly consistent
      read is used; otherwise, an eventually consistent read is used.</p>")
  @as("ConsistentRead")
  consistentRead: option<consistentRead>,
  @ocaml.doc("<p>This is a legacy parameter.  Use <code>ProjectionExpression</code> instead.  For more information, see
...
module DescribeLimits = {
  ...
  type response = {
    ...
    @ocaml.doc("<p>The maximum total write capacity units that your account allows you to provision across
...
module ListTables = {
  ...
  type request = {
    ...
    @ocaml.doc("<p>The first table name that this operation will evaluate. Use the value that was returned for
  ...
  type response = {
    ...
    @ocaml.doc("<p>The names of the tables associated with the current account at the current endpoint. The maximum size of this array is 100.</p>
...
module ExportTableToPointInTime = {
  ...
  type request = {
    ...
    @as("ExportFormat")
    exportFormat: option<exportFormat>,
    @ocaml.doc("<p>The ID of the AWS KMS managed key used to encrypt the S3 bucket where export data will
...
module DescribeContributorInsights = {
  ...
  type response = {
    @ocaml.doc("<p>Returns information about the last failure that encountered.</p>
    ...
    @ocaml.doc("<p>Current Status contributor insights.</p>") @as("ContributorInsightsStatus")
    contributorInsightsStatus: option<contributorInsightsStatus>,
    @ocaml.doc("<p>List of names of the associated Alpine rules.</p>")
...
module ListTagsOfResource = {
  ...
  type response = {
    @ocaml.doc("<p>If this value is returned, there are additional results to be displayed. To retrieve them,
...
module ListGlobalTables = {
  ...
  type request = {
    ...
    regionName: option<regionName>,
    @ocaml.doc("<p>The maximum number of table names to return, if the parameter is not specified DynamoDB defaults to 100.</p>
...
module UpdateTable = {
  ...
  type request = {
    ...
    @as("StreamSpecification")
    streamSpecification: option<streamSpecification>,
    @ocaml.doc("<p>An array of one or more global secondary indexes for the table. For each index in the array, you can request one action:</p>
         <ul>
            <li>
               <p>
    ...
    @as("ProvisionedThroughput")
    provisionedThroughput: option<provisionedThroughput>,
    @ocaml.doc("<p>Controls how you are charged for read and write throughput and how you manage capacity. 
...
module RestoreTableToPointInTime = {
  ...
  type request = {
    ...
    @ocaml.doc("<p>List of local secondary indexes for the restored table. The indexes
    ...
    @ocaml.doc("<p>List of global secondary indexes for the restored table. The indexes
        provided should match existing secondary indexes. You can choose to exclude
        some or all of the indexes at the time of restore.</p>")
    ...
    @ocaml.doc("<p>The DynamoDB table that will be restored. This value is an Amazon
            Resource Name (ARN).</p>")
    @as("SourceTableArn")
    sourceTableArn: option<tableArn>,
  }
...
module CreateTable = {
  ...
  type request = {
    ...
    @as("BillingMode")
    billingMode: option<billingMode>,
    @ocaml.doc("<p>One or more global secondary indexes (the maximum is 20) to be created on the table. Each global secondary index in the array includes the following:</p>
         <ul>
            <li>
               <p>
    ...
    @ocaml.doc("<p>One or more local secondary indexes (the maximum is 5) to be created on the table. Each index is scoped to a given partition key value. There is a 10 GB size limit per partition key value; otherwise, the size of a local secondary index is unconstrained.</p>
    ...
    @ocaml.doc("<p>Specifies the attributes that make up the primary key for a table or an index. The attributes
...
module UpdateGlobalTableSettings = {
  ...
  type request = {
    ...
    @ocaml.doc("<p>The billing mode of the global table. If <code>GlobalTableBillingMode</code> is not specified, the global table defaults to <code>PROVISIONED</code> capacity billing mode.</p>
         <ul>
            <li>
               <p>
                  <code>PROVISIONED</code> - We recommend using <code>PROVISIONED</code> for predictable workloads. <code>PROVISIONED</code> sets the billing mode to <a href=\"https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/HowItWorks.ReadWriteCapacityMode.html#HowItWorks.ProvisionedThroughput.Manual\">Provisioned Mode</a>.</p>
...
module UpdateTableReplicaAutoScaling = {
  ...
  type request = {
    ...
    tableName: tableName,
    @ocaml.doc("<p>Represents the auto scaling settings of the global secondary indexes of the replica
          to be updated.</p>")
    @as("GlobalSecondaryIndexUpdates")
...
module GetItem = {
  ...
  type request = {
    ...
    @as("ExpressionAttributeNames")
    expressionAttributeNames: option<expressionAttributeNameMap>,
    @ocaml.doc("<p>A string that identifies one or more attributes to retrieve from the table. These attributes can include scalars, sets, or elements of a JSON document. The attributes in the expression must be separated by commas.</p>
    ...
    @as("AttributesToGet")
    attributesToGet: option<attributeNameList>,
    @ocaml.doc("<p>A map of attribute names to <code>AttributeValue</code> objects, representing the primary key of
  ...
  type response = {
    ...
    @ocaml.doc("<p>A map of attribute names to <code>AttributeValue</code> objects, as specified
...
If a read operation was used, this property will contain the result of the reade operation; a map of attribute names and their values. For the write operations this value will be empty.
  ...
  let make = (~statement, ~nextToken=?, ~consistentRead=?, ~parameters=?, ()) =>
    new({
      nextToken: nextToken,
      consistentRead: consistentRead,
...
module UpdateItem = {
  ...
  type request = {
    ...
    @as("ExpressionAttributeNames")
    expressionAttributeNames: option<expressionAttributeNameMap>,
    @ocaml.doc("<p>A condition that must be satisfied in order for a conditional update to succeed.</p>
    ...
    @ocaml.doc("<p>Determines whether item collection metrics are returned.  If set to <code>SIZE</code>, the response includes statistics about item collections, if any, that were modified during
    ...
    @ocaml.doc("<p>This is a legacy parameter.  Use <code>ConditionExpression</code> instead.   For more information, see
    ...
    @ocaml.doc("<p>This is a legacy parameter.  Use <code>UpdateExpression</code> instead.   For more information, see
    ...
    @ocaml.doc("<p>The primary key of the item to be updated. Each element consists of an attribute name and a value for that attribute.</p>
         <p>For the primary key, you must provide all of the attributes. For example, with a simple primary key, you only need to provide a value for the partition key. For a composite primary key, you must provide values for both the partition key and the sort key.</p>")
    @as("Key")
    key: key,
  ...
  type response = {
    ...
    @ocaml.doc("<p>The capacity units consumed by the <code>UpdateItem</code> operation. The data returned includes the total
            provisioned throughput consumed, along with statistics for the table and any indexes involved
...
module Scan = {
  ...
  type request = {
    ...
    @as("ExpressionAttributeNames")
    expressionAttributeNames: option<expressionAttributeNameMap>,
    @ocaml.doc("<p>A string that contains conditions that DynamoDB applies after the <code>Scan</code> operation, but
    ...
    @ocaml.doc("<p>A string that identifies one or more attributes to retrieve from the specified table or index. These attributes can include scalars, sets, or elements of a JSON document. The attributes in the expression must be separated by commas.</p>
         <p>If no attribute names are specified, then all attributes will be returned. If any of the requested attributes are not found, they will not appear in the result.</p>
         <p>For more information, see <a href=\"https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Expressions.AccessingItemAttributes.html\">Specifying Item Attributes</a> in the <i>Amazon DynamoDB Developer
                Guide</i>.</p>")
    @as("ProjectionExpression")
    projectionExpression: option<projectionExpression>,
    @ocaml.doc("<p>For a parallel <code>Scan</code> request, <code>Segment</code> identifies an individual segment to be
    ...
    @ocaml.doc("<p>For a parallel <code>Scan</code> request, <code>TotalSegments</code> represents the total number of
    ...
    @as("ReturnConsumedCapacity") returnConsumedCapacity: option<returnConsumedCapacity>,
    @ocaml.doc("<p>The primary key of the first item that this operation will evaluate. Use the value that was returned for <code>LastEvaluatedKey</code> in the previous operation.</p>
    ...
    @ocaml.doc("<p>This is a legacy parameter.  Use <code>FilterExpression</code> instead.   For more information, see
          <a href=\"https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/LegacyConditionalParameters.ConditionalOperator.html\">ConditionalOperator</a> in the <i>Amazon DynamoDB Developer Guide</i>.</p>")
    @as("ConditionalOperator")
    conditionalOperator: option<conditionalOperator>,
    @ocaml.doc("<p>This is a legacy parameter.  Use <code>FilterExpression</code> instead.   For more information, see
    ...
    @ocaml.doc("<p>The attributes to be returned in the
    ...
    @ocaml.doc("<p>The maximum number of items to evaluate (not necessarily the number of matching items).
  ...
  type response = {
    @ocaml.doc("<p>The capacity units consumed by the <code>Scan</code> operation. The data returned includes the total
            provisioned throughput consumed, along with statistics for the table and any indexes involved
            in the operation. <code>ConsumedCapacity</code> is only returned if the <code>ReturnConsumedCapacity</code> parameter was specified.
            For more information, see <a href=\"https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/ProvisionedThroughputIntro.html\">Provisioned
                Throughput</a> in the <i>Amazon DynamoDB Developer Guide</i>.</p>")
...
module Query = {
  ...
  type request = {
    ...
    @ocaml.doc("<p>A string that contains conditions that DynamoDB applies after the <code>Query</code> operation, but
       before the data is returned to you. Items that do not satisfy the <code>FilterExpression</code>
       criteria are not returned.</p>
    ...
    @as("ExclusiveStartKey")
    exclusiveStartKey: option<key>,
    @ocaml.doc("<p>Specifies the order for index traversal: If <code>true</code> (default), the traversal is performed in ascending order; if <code>false</code>, the traversal is performed in descending order. </p>
    ...
    @ocaml.doc("<p>This is a legacy parameter.  Use <code>KeyConditionExpression</code> instead.   For more information, see
    ...
    @ocaml.doc("<p>Determines the read consistency model:  If set to <code>true</code>, then the operation uses strongly consistent reads; otherwise, the operation uses eventually consistent reads.</p>
    ...
    @as("Select")
    select: option<select>,
    @ocaml.doc("<p>The name of an index to query. This index can be any local secondary index or global secondary index on the table. Note that
  ...
  type response = {
    ...
    @as("ConsumedCapacity")
    consumedCapacity: option<consumedCapacity>,
    @ocaml.doc("<p>The primary key of the item where the operation stopped, inclusive of the previous result set. Use this value to start a new operation, excluding this value in the new request.</p>
...
module PutItem = {
  ...
  type request = {
    ...
    @as("ExpressionAttributeNames")
    expressionAttributeNames: option<expressionAttributeNameMap>,
    @ocaml.doc("<p>A condition that must be satisfied in order for a conditional <code>PutItem</code> operation to
      succeed.</p>
         <p>An expression can contain any of the following:</p>
         <ul>
            <li>
    ...
    returnItemCollectionMetrics: option<returnItemCollectionMetrics>,
    @as("ReturnConsumedCapacity") returnConsumedCapacity: option<returnConsumedCapacity>,
    @ocaml.doc("<p>Use <code>ReturnValues</code> if you want to get the item attributes as they appeared before they
    ...
    @as("Expected")
    expected: option<expectedAttributeMap>,
    @ocaml.doc("<p>A map of attribute name/value pairs, one for each attribute. Only the primary key attributes are required; you can optionally provide other attribute name-value pairs for the item.</p>
  ...
  type response = {
    ...
    @as("ConsumedCapacity")
    consumedCapacity: option<consumedCapacity>,
    @ocaml.doc("<p>The attribute values as they appeared before the <code>PutItem</code> operation, but only if
...
The response to a PartiQL transaction.
  ...
  let make = (~transactStatements, ~clientRequestToken=?, ()) =>
    new({clientRequestToken: clientRequestToken, transactStatements: transactStatements})
  @send external send: (awsServiceClient, t) => Js.Promise.t<response> = "send"
}
...
module DeleteItem = {
  ...
  type request = {
    ...
    @as("ExpressionAttributeNames")
    expressionAttributeNames: option<expressionAttributeNameMap>,
    @ocaml.doc("<p>A condition that must be satisfied in order for a conditional <code>DeleteItem</code> to
      succeed.</p>
         <p>An expression can contain any of the following:</p>
         <ul>
            <li>
  ...
  type response = {
    ...
    @as("ConsumedCapacity")
    consumedCapacity: option<consumedCapacity>,
    @ocaml.doc("<p>A map of attribute names to <code>AttributeValue</code> objects, representing the item as it
...
The response to each PartiQL statement in the batch.
  ...
  let make = (~statements, ()) => new({statements: statements})
  @send external send: (awsServiceClient, t) => Js.Promise.t<response> = "send"
}
...
module TransactWriteItems = {
  type t
  type request = {
    @ocaml.doc("<p>Providing a <code>ClientRequestToken</code> makes the call to <code>TransactWriteItems</code>
    ...
    @as("ClientRequestToken")
    clientRequestToken: option<clientRequestToken>,
    @ocaml.doc("<p>Determines whether item collection metrics are returned. If set to
  ...
  type response = {
    ...
    @ocaml.doc("<p>The capacity units consumed by the entire <code>TransactWriteItems</code>
...
module TransactGetItems = {
  type t
  type request = {
    @ocaml.doc("<p>A value of <code>TOTAL</code> causes consumed capacity information
    ...
    @ocaml.doc("<p>An ordered array of up to 25 <code>TransactGetItem</code> objects,
         each of which contains a <code>Get</code> structure.</p>")
    @as("TransactItems")
    transactItems: transactGetItemList,
  }
  type response = {
    @ocaml.doc("<p>An ordered array of up to 25 <code>ItemResponse</code> objects, each of which corresponds
    ...
    @as("Responses")
    responses: option<itemResponseList>,
    @ocaml.doc("<p>If the <i>ReturnConsumedCapacity</i> value was <code>TOTAL</code>,
...
module BatchGetItem = {
  ...
  type request = {
    @as("ReturnConsumedCapacity") returnConsumedCapacity: option<returnConsumedCapacity>,
    @ocaml.doc("<p>A map of one or more table names and, for each table, a map that describes one or more items to retrieve from that table. Each table name can be used only once per <code>BatchGetItem</code> request.</p>
  ...
  type response = {
    @ocaml.doc("<p>The read capacity units consumed by the entire <code>BatchGetItem</code> operation.</p>
    ...
    @as("ConsumedCapacity")
    consumedCapacity: option<consumedCapacityMultiple>,
    @ocaml.doc("<p>A map of tables and their respective keys that were not processed with the current response.
    ...
    @ocaml.doc("<p>A map of table name to a list of items. Each object in <code>Responses</code> consists of a table
...
module BatchWriteItem = {
  ...
  type request = {
    ...
    returnItemCollectionMetrics: option<returnItemCollectionMetrics>,
    @as("ReturnConsumedCapacity") returnConsumedCapacity: option<returnConsumedCapacity>,
    @ocaml.doc("<p>A map of one or more table names and, for each table, a list of operations to be performed
  ...
  type response = {
    @ocaml.doc("<p>The capacity units consumed by the entire <code>BatchWriteItem</code> operation.</p>
         <p>Each element consists of:</p>
         <ul>
            <li>
               <p>
    ...
    @as("ConsumedCapacity")
    consumedCapacity: option<consumedCapacityMultiple>,
    @ocaml.doc("<p>A list of tables that were processed by <code>BatchWriteItem</code> and, for each table,
