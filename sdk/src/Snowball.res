...
@ocaml.doc("<p>Configures the wireless connection on an AWS Snowcone device.</p>")
type wirelessConnection = {
  @ocaml.doc("<p>Enables the Wi-Fi adapter on an AWS Snowcone device.</p>") @as("IsWifiEnabled")
...
@ocaml.doc("<p>Contains job logs. Whenever a Snow device is used to import data into or export data out of
...
type jobListEntry = {
  ...
  @ocaml.doc("<p>A value that indicates that this job is a main job. A main job represents a
...
@ocaml.doc("<p>The tax documents required in AWS Regions in India.</p>")
type indtaxDocuments = {
  @ocaml.doc("<p>The Goods and Services Tax (GST) documents required in AWS Regions in India.</p>")
  @as("GSTIN")
  gstin: option<gstin>,
}
@ocaml.doc("<p>The container for the <a>EventTriggerDefinition$EventResourceARN</a>.</p>")
type eventTriggerDefinition = {
  @ocaml.doc("<p>The Amazon Resource Name (ARN) for any local Amazon S3 resource that is an AWS Lambda
...
@ocaml.doc("<p>Defines the real-time status of a Snow device's data transfer while the device is at AWS.
...
type dataTransfer = {
  @ocaml.doc("<p>The total number of objects for a transfer between a Snow device and Amazon S3. This value
      is set to 0 (zero) until all the keys that will be transferred have been listed.</p>")
  @as("TotalObjects")
  totalObjects: option<long>,
  @ocaml.doc("<p>The total bytes of data for a transfer between a Snow device and Amazon S3. This value is
...
@ocaml.doc("<p>The address that you want the Snow device(s) associated with a specific job to
...
@ocaml.doc("<p>The tax documents required in your AWS Region.</p>")
type taxDocuments = {@as("IND") ind: option<indtaxDocuments>}
@ocaml.doc("<p>Specifies the device configuration for an AWS Snowcone job. </p>")
type snowconeDeviceConfiguration = {
  @ocaml.doc("<p>Configures the wireless connection for the AWS Snowcone device.</p>")
...
type lambdaResource = {
  ...
  @ocaml.doc("<p>An Amazon Resource Name (ARN) that represents an AWS Lambda function to be triggered by
...
type deviceConfiguration = {
  @ocaml.doc("<p>Returns information about the device configuration for an AWS Snowcone job.</p>")
...
@ocaml.doc("<p>Contains an array of AWS resource objects. Each object represents an Amazon S3 bucket,
...
type jobMetadata = {
  ...
  @ocaml.doc("<p>The metadata associated with the tax documents required in your AWS Region.</p>")
  @as("TaxDocuments")
  ...
  @ocaml.doc("<p>The Amazon Resource Name (ARN) for the AWS Key Management Service (AWS KMS) key
...
type clusterMetadata = {
  @ocaml.doc("<p>The tax documents required in your AWS Region.</p>") @as("TaxDocuments")
  taxDocuments: option<taxDocuments>,
  ...
  @ocaml.doc("<p>The creation date for this cluster.</p>") @as("CreationDate")
  creationDate: option<timestamp_>,
  @ocaml.doc("<p>The type of AWS Snow device to use for this cluster. 
      </p>
         <note>
...
@ocaml.doc("<p>AWS Snow Family is a petabyte-scale data transport solution that uses secure devices to
...
module CreateReturnShippingLabel = {
  type t
  type request = {
    @ocaml.doc("<p>The shipping speed for a particular job. This speed doesn't dictate how soon the device is returned to AWS.
...
module CreateLongTermPricing = {
  type t
  type request = {
    @ocaml.doc("<p>The type of AWS Snow Family device to use for the long term pricing job.</p>")
    @as("SnowballType")
    snowballType: option<snowballType>,
    @ocaml.doc(
...
module CreateJob = {
  ...
  type request = {
    ...
    @as("LongTermPricingId")
    longTermPricingId: option<longTermPricingId>,
    @ocaml.doc("<p>Defines the device configuration for an AWS Snowcone job.</p>
    ...
    @as("ForwardingAddressId")
    forwardingAddressId: option<addressId>,
    @ocaml.doc("<p>The type of AWS Snow Family device to use for this job. 
      </p>
         <note>
            <p>For cluster jobs, AWS Snow Family currently supports only the <code>EDGE</code> device type.</p>
         </note>
...
module CreateCluster = {
  ...
  type request = {
    ...
    @as("ShippingOption")
    shippingOption: shippingOption,
    @ocaml.doc("<p>The type of AWS Snow Family device to use for this cluster. 
    </p>
         <note>
            <p>For cluster jobs, AWS Snow Family currently supports only the <code>EDGE</code> device type.</p>
         </note>
