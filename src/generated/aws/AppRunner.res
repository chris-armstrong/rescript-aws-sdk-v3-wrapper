...
type serviceStatus = [@as("OPERATION_IN_PROGRESS") #OPERATIONINPROGRESS | @as("PAUSED") #PAUSED | @as("DELETE_FAILED") #DELETEFAILED | @as("DELETED") #DELETED | @as("RUNNING") #RUNNING | @as("CREATE_FAILED") #CREATEFAILED]
type serviceName = string
type serviceMaxResults = int
type serviceId = string
type runtimeEnvironmentVariablesValue = string
type runtimeEnvironmentVariablesKey = string
type runtime = [@as("NODEJS_12") #NODEJS12 | @as("PYTHON_3") #PYTHON3]
type roleArn = string
type providerType = [@as("GITHUB") #GITHUB]
type operationType = [@as("DELETE_SERVICE") #DELETESERVICE | @as("RESUME_SERVICE") #RESUMESERVICE | @as("PAUSE_SERVICE") #PAUSESERVICE | @as("CREATE_SERVICE") #CREATESERVICE | @as("START_DEPLOYMENT") #STARTDEPLOYMENT]
type operationStatus = [@as("ROLLBACK_SUCCEEDED") #ROLLBACKSUCCEEDED | @as("ROLLBACK_FAILED") #ROLLBACKFAILED | @as("ROLLBACK_IN_PROGRESS") #ROLLBACKINPROGRESS | @as("SUCCEEDED") #SUCCEEDED | @as("FAILED") #FAILED | @as("IN_PROGRESS") #INPROGRESS | @as("PENDING") #PENDING]
...
type integer_ = int
type imageRepositoryType = [@as("ECR_PUBLIC") #ECRPUBLIC | @as("ECR") #ECR]
...
type customDomainAssociationStatus = [@as("BINDING_CERTIFICATE") #BINDINGCERTIFICATE | @as("PENDING_CERTIFICATE_DNS_VALIDATION") #PENDINGCERTIFICATEDNSVALIDATION | @as("DELETE_FAILED") #DELETEFAILED | @as("DELETING") #DELETING | @as("ACTIVE") #ACTIVE | @as("CREATE_FAILED") #CREATEFAILED | @as("CREATING") #CREATING]
type cpu = string
type connectionStatus = [@as("DELETED") #DELETED | @as("ERROR") #ERROR | @as("AVAILABLE") #AVAILABLE | @as("PENDING_HANDSHAKE") #PENDINGHANDSHAKE]
type connectionName = string
type configurationSource = [@as("API") #API | @as("REPOSITORY") #REPOSITORY]
type certificateValidationRecordStatus = [@as("FAILED") #FAILED | @as("SUCCESS") #SUCCESS | @as("PENDING_VALIDATION") #PENDINGVALIDATION]
