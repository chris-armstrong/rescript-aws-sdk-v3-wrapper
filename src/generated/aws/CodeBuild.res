...
type webhookFilterType = [@as("COMMIT_MESSAGE") #COMMITMESSAGE | @as("FILE_PATH") #FILEPATH | @as("ACTOR_ACCOUNT_ID") #ACTORACCOUNTID | @as("HEAD_REF") #HEADREF | @as("BASE_REF") #BASEREF | @as("EVENT") #EVENT]
type webhookBuildType = [@as("BUILD_BATCH") #BUILDBATCH | @as("BUILD") #BUILD]
type valueInput = string
type timestamp_ = Js.Date.t;
type timeOut = int
type string_ = string
type statusType = [@as("STOPPED") #STOPPED | @as("IN_PROGRESS") #INPROGRESS | @as("TIMED_OUT") #TIMEDOUT | @as("FAULT") #FAULT | @as("FAILED") #FAILED | @as("SUCCEEDED") #SUCCEEDED]
type sourceType = [@as("NO_SOURCE") #NOSOURCE | @as("GITHUB_ENTERPRISE") #GITHUBENTERPRISE | @as("BITBUCKET") #BITBUCKET | @as("S3") #S3 | @as("GITHUB") #GITHUB | @as("CODEPIPELINE") #CODEPIPELINE | @as("CODECOMMIT") #CODECOMMIT]
type sourceAuthType = [@as("OAUTH") #OAUTH]
type sortOrderType = [@as("DESCENDING") #DESCENDING | @as("ASCENDING") #ASCENDING]
type sharedResourceSortByType = [@as("MODIFIED_TIME") #MODIFIEDTIME | @as("ARN") #ARN]
type serverType = [@as("GITHUB_ENTERPRISE") #GITHUBENTERPRISE | @as("BITBUCKET") #BITBUCKET | @as("GITHUB") #GITHUB]
type sensitiveNonEmptyString = string
type retryBuildBatchType = [@as("RETRY_FAILED_BUILDS") #RETRYFAILEDBUILDS | @as("RETRY_ALL_BUILDS") #RETRYALLBUILDS]
type reportType = [@as("CODE_COVERAGE") #CODECOVERAGE | @as("TEST") #TEST]
...
type reportGroupTrendFieldType = [@as("BRANCHES_MISSED") #BRANCHESMISSED | @as("BRANCHES_COVERED") #BRANCHESCOVERED | @as("BRANCH_COVERAGE") #BRANCHCOVERAGE | @as("LINES_MISSED") #LINESMISSED | @as("LINES_COVERED") #LINESCOVERED | @as("LINE_COVERAGE") #LINECOVERAGE | @as("TOTAL") #TOTAL | @as("DURATION") #DURATION | @as("PASS_RATE") #PASSRATE]
type reportGroupStatusType = [@as("DELETING") #DELETING | @as("ACTIVE") #ACTIVE]
type reportGroupSortByType = [@as("LAST_MODIFIED_TIME") #LASTMODIFIEDTIME | @as("CREATED_TIME") #CREATEDTIME | @as("NAME") #NAME]
type reportGroupName = string
type reportExportConfigType = [@as("NO_EXPORT") #NOEXPORT | @as("S3") #S3]
type reportCodeCoverageSortByType = [@as("FILE_PATH") #FILEPATH | @as("LINE_COVERAGE_PERCENTAGE") #LINECOVERAGEPERCENTAGE]
type projectSortByType = [@as("LAST_MODIFIED_TIME") #LASTMODIFIEDTIME | @as("CREATED_TIME") #CREATEDTIME | @as("NAME") #NAME]
type projectName = string
type projectDescription = string
type platformType = [@as("WINDOWS_SERVER") #WINDOWSSERVER | @as("UBUNTU") #UBUNTU | @as("AMAZON_LINUX") #AMAZONLINUX | @as("DEBIAN") #DEBIAN]
type percentage = float
type pageSize = int
type nonNegativeInt = int
type nonEmptyString = string
type logsConfigStatusType = [@as("DISABLED") #DISABLED | @as("ENABLED") #ENABLED]
type languageType = [@as("PHP") #PHP | @as("BASE") #BASE | @as("DOTNET") #DOTNET | @as("ANDROID") #ANDROID | @as("DOCKER") #DOCKER | @as("GOLANG") #GOLANG | @as("RUBY") #RUBY | @as("NODE_JS") #NODEJS | @as("PYTHON") #PYTHON | @as("JAVA") #JAVA]
type keyInput = string
type imagePullCredentialsType = [@as("SERVICE_ROLE") #SERVICEROLE | @as("CODEBUILD") #CODEBUILD]
type gitCloneDepth = int
type fileSystemType = [@as("EFS") #EFS]
type environmentVariableType = [@as("SECRETS_MANAGER") #SECRETSMANAGER | @as("PARAMETER_STORE") #PARAMETERSTORE | @as("PLAINTEXT") #PLAINTEXT]
type environmentType = [@as("WINDOWS_SERVER_2019_CONTAINER") #WINDOWSSERVER2019CONTAINER | @as("ARM_CONTAINER") #ARMCONTAINER | @as("LINUX_GPU_CONTAINER") #LINUXGPUCONTAINER | @as("LINUX_CONTAINER") #LINUXCONTAINER | @as("WINDOWS_CONTAINER") #WINDOWSCONTAINER]
type credentialProviderType = [@as("SECRETS_MANAGER") #SECRETSMANAGER]
type computeType = [@as("BUILD_GENERAL1_2XLARGE") #BUILDGENERAL12XLARGE | @as("BUILD_GENERAL1_LARGE") #BUILDGENERAL1LARGE | @as("BUILD_GENERAL1_MEDIUM") #BUILDGENERAL1MEDIUM | @as("BUILD_GENERAL1_SMALL") #BUILDGENERAL1SMALL]
type cacheType = [@as("LOCAL") #LOCAL | @as("S3") #S3 | @as("NO_CACHE") #NOCACHE]
type cacheMode = [@as("LOCAL_CUSTOM_CACHE") #LOCALCUSTOMCACHE | @as("LOCAL_SOURCE_CACHE") #LOCALSOURCECACHE | @as("LOCAL_DOCKER_LAYER_CACHE") #LOCALDOCKERLAYERCACHE]
type buildPhaseType = [@as("COMPLETED") #COMPLETED | @as("FINALIZING") #FINALIZING | @as("UPLOAD_ARTIFACTS") #UPLOADARTIFACTS | @as("POST_BUILD") #POSTBUILD | @as("BUILD") #BUILD | @as("PRE_BUILD") #PREBUILD | @as("INSTALL") #INSTALL | @as("DOWNLOAD_SOURCE") #DOWNLOADSOURCE | @as("PROVISIONING") #PROVISIONING | @as("QUEUED") #QUEUED | @as("SUBMITTED") #SUBMITTED]
type buildBatchPhaseType = [@as("STOPPED") #STOPPED | @as("FAILED") #FAILED | @as("SUCCEEDED") #SUCCEEDED | @as("COMBINE_ARTIFACTS") #COMBINEARTIFACTS | @as("IN_PROGRESS") #INPROGRESS | @as("DOWNLOAD_BATCHSPEC") #DOWNLOADBATCHSPEC | @as("SUBMITTED") #SUBMITTED]
type bucketOwnerAccess = [@as("FULL") #FULL | @as("READ_ONLY") #READONLY | @as("NONE") #NONE]
type boolean_ = bool
type authType = [@as("PERSONAL_ACCESS_TOKEN") #PERSONALACCESSTOKEN | @as("BASIC_AUTH") #BASICAUTH | @as("OAUTH") #OAUTH]
type artifactsType = [@as("NO_ARTIFACTS") #NOARTIFACTS | @as("S3") #S3 | @as("CODEPIPELINE") #CODEPIPELINE]
type artifactPackaging = [@as("ZIP") #ZIP | @as("NONE") #NONE]
type artifactNamespace = [@as("BUILD_ID") #BUILDID | @as("NONE") #NONE]
type webhookFilter = {
