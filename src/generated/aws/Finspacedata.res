...
type errorCategory = [@as("A_user_recoverable_error_has_occurred") #AUserRecoverableErrorHasOccurred | @as("Cancelled") #Cancelled | @as("An_internal_error_has_occurred") #AnInternalErrorHasOccurred | @as("The_system_temporarily_lacks_sufficient_resources_to_process_the_request") #TheSystemTemporarilyLacksSufficientResourcesToProcessTheRequest | @as("One_or_more_inputs_to_this_request_were_not_found") #OneOrMoreInputsToThisRequestWereNotFound | @as("Missing_required_permission_to_perform_this_request") #MissingRequiredPermissionToPerformThisRequest | @as("Service_limits_have_been_exceeded") #ServiceLimitsHaveBeenExceeded | @as("The_inputs_to_this_request_are_invalid") #TheInputsToThisRequestAreInvalid]
type changesetStatus = [@as("STOP_REQUESTED") #STOPREQUESTED | @as("RUNNING") #RUNNING | @as("SUCCESS") #SUCCESS | @as("FAILED") #FAILED | @as("PENDING") #PENDING]
...
@module("@aws-sdk/client-finspace-api") @new external createClient: unit => awsServiceClient = "FinspacedataClient";
...
datasetId: idType
  ...
  type response = {
changeset: option<changesetInfo>
}
