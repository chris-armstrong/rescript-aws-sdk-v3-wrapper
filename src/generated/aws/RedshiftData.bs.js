// Generated by ReScript, PLEASE EDIT WITH CARE
'use strict';

var Caml_option = require("rescript/lib/js/caml_option.js");
var Caml_exceptions = require("rescript/lib/js/caml_exceptions.js");

var FieldUnspecified = /* @__PURE__ */Caml_exceptions.create("RedshiftData-AwsSdkV3.Field.FieldUnspecified");

function classify(value) {
  var x = value.blobValue;
  if (x !== undefined) {
    return {
            TAG: /* BlobValue */0,
            _0: Caml_option.valFromOption(x)
          };
  }
  var x$1 = value.stringValue;
  if (x$1 !== undefined) {
    return {
            TAG: /* StringValue */1,
            _0: x$1
          };
  }
  var x$2 = value.doubleValue;
  if (x$2 !== undefined) {
    return {
            TAG: /* DoubleValue */2,
            _0: x$2
          };
  }
  var x$3 = value.longValue;
  if (x$3 !== undefined) {
    return {
            TAG: /* LongValue */3,
            _0: x$3
          };
  }
  var x$4 = value.booleanValue;
  if (x$4 !== undefined) {
    return {
            TAG: /* BooleanValue */4,
            _0: x$4
          };
  }
  var x$5 = value.isNull;
  if (x$5 !== undefined) {
    return {
            TAG: /* IsNull */5,
            _0: x$5
          };
  }
  throw {
        RE_EXN_ID: FieldUnspecified,
        Error: new Error()
      };
}

function make(value) {
  switch (value.TAG | 0) {
    case /* BlobValue */0 :
        return {
                blobValue: Caml_option.some(value._0),
                stringValue: undefined,
                doubleValue: undefined,
                longValue: undefined,
                booleanValue: undefined,
                isNull: undefined
              };
    case /* StringValue */1 :
        return {
                blobValue: undefined,
                stringValue: value._0,
                doubleValue: undefined,
                longValue: undefined,
                booleanValue: undefined,
                isNull: undefined
              };
    case /* DoubleValue */2 :
        return {
                blobValue: undefined,
                stringValue: undefined,
                doubleValue: value._0,
                longValue: undefined,
                booleanValue: undefined,
                isNull: undefined
              };
    case /* LongValue */3 :
        return {
                blobValue: undefined,
                stringValue: undefined,
                doubleValue: undefined,
                longValue: value._0,
                booleanValue: undefined,
                isNull: undefined
              };
    case /* BooleanValue */4 :
        return {
                blobValue: undefined,
                stringValue: undefined,
                doubleValue: undefined,
                longValue: undefined,
                booleanValue: value._0,
                isNull: undefined
              };
    case /* IsNull */5 :
        return {
                blobValue: undefined,
                stringValue: undefined,
                doubleValue: undefined,
                longValue: undefined,
                booleanValue: undefined,
                isNull: value._0
              };
    
  }
}

var Field = {
  FieldUnspecified: FieldUnspecified,
  classify: classify,
  make: make
};

var ExecuteStatement = {};

var DescribeStatement = {};

var CancelStatement = {};

var ListSchemas = {};

var ListDatabases = {};

var ListTables = {};

var ListStatements = {};

var DescribeTable = {};

var GetStatementResult = {};

exports.Field = Field;
exports.ExecuteStatement = ExecuteStatement;
exports.DescribeStatement = DescribeStatement;
exports.CancelStatement = CancelStatement;
exports.ListSchemas = ListSchemas;
exports.ListDatabases = ListDatabases;
exports.ListTables = ListTables;
exports.ListStatements = ListStatements;
exports.DescribeTable = DescribeTable;
exports.GetStatementResult = GetStatementResult;
/* No side effect */
