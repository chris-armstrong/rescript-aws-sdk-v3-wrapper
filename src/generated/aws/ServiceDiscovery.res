...
type serviceType = [@as("DNS") #DNS | @as("DNS_HTTP") #DNSHTTP | @as("HTTP") #HTTP]
type serviceName = string
type serviceFilterName = [@as("NAMESPACE_ID") #NAMESPACEID]
...
type operationType = [@as("DEREGISTER_INSTANCE") #DEREGISTERINSTANCE | @as("REGISTER_INSTANCE") #REGISTERINSTANCE | @as("UPDATE_SERVICE") #UPDATESERVICE | @as("DELETE_NAMESPACE") #DELETENAMESPACE | @as("CREATE_NAMESPACE") #CREATENAMESPACE]
...
type operationFilterName = [@as("UPDATE_DATE") #UPDATEDATE | @as("TYPE") #TYPE | @as("STATUS") #STATUS | @as("SERVICE_ID") #SERVICEID | @as("NAMESPACE_ID") #NAMESPACEID]
type nextToken = string
type namespaceType = [@as("HTTP") #HTTP | @as("DNS_PRIVATE") #DNSPRIVATE | @as("DNS_PUBLIC") #DNSPUBLIC]
...
type healthStatusFilter = [@as("HEALTHY_OR_ELSE_ALL") #HEALTHYORELSEALL | @as("ALL") #ALL | @as("UNHEALTHY") #UNHEALTHY | @as("HEALTHY") #HEALTHY]
...
type httpProperties = {
@as("HttpName") httpName: option<namespaceName>
}
...
@module("@aws-sdk/client-servicediscovery") @new external createClient: unit => awsServiceClient = "ServiceDiscoveryClient";
...
module GetOperation = {
  type t;
  type request = {
@as("OperationId") operationId: resourceId
}
  type response = {
...
@as("NamespaceName") namespaceName: namespaceName
  ...
  type response = {
@as("Instances") instances: option<httpInstanceSummaryList>
}
