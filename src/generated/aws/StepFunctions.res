...
type arn = string
type tracingConfiguration = {enabled: option<enabled>}
...
type mapStateStartedEventDetails = {length: option<unsignedInteger>}
...
type historyEventExecutionDataDetails = {truncated: option<truncated>}
...
type cloudWatchLogsLogGroup = {logGroupArn: option<arn>}
type cloudWatchEventsExecutionDataDetails = {included: option<includedDetails>}
type billingDetails = {
...
type activityStartedEventDetails = {workerName: option<identity>}
...
type logDestination = {cloudWatchLogsLogGroup: option<cloudWatchLogsLogGroup>}
...
module StopExecution = {
  ...
  type response = {stopDate: timestamp_}
...
module SendTaskHeartbeat = {
  type t
  type request = {taskToken: taskToken}
  type response = unit
...
module DeleteStateMachine = {
  type t
  type request = {stateMachineArn: arn}
  type response = unit
...
module UpdateStateMachine = {
  ...
  type response = {updateDate: timestamp_}
...
module DescribeStateMachineForExecution = {
  ...
  @module("@aws-sdk/client-states") @new
  external new_: request => t = "DescribeStateMachineForExecutionCommand"
  @send external rawSend: (awsServiceClient, t) => Js.Promise.t<response> = "send"
}
